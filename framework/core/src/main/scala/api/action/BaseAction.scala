...
trait BaseAction extends FutureAction with Log with SkipCsrfCheck {
  ...
  lazy val config = DiddConfigFactory.config
  ...
  lazy val devMode  = DiddConfigFactory.env match {
    case "dev-local" => true
    case _ => false
  }

  lazy val env = DiddConfigFactory.env
...
trait JsonAction extends BaseAction with SkipCsrfCheck {
  ...
  private[this] def respondException(e: Throwable) = {
    ...
    val message = underlyingException match {
      case Some(exc) => if (DiddConfigFactory.env == "dev-local") {
        Map(
    ...
    if (DiddConfigFactory.env != "dev-local") {
      Future {
        val identity = user match {
          ...
          case _ => {
            val i = new RaygunIdentifier("anonymous@didd.com")
        ...
        tags.add(DiddConfigFactory.env)
        tags.add("scala")
...
trait BasicAuthAction extends BaseAction {
  beforeFilter {
    basicAuth("Didd Restricted") { (username, password) =>
