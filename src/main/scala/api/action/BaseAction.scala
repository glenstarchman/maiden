...
trait BaseAction extends FutureAction with Log with SkipCsrfCheck {
  ...
  lazy val config = MaidenConfigFactory.config
  ...
  lazy val devMode  = MaidenConfigFactory.env match {
    case "dev-local" => true
    case _ => false
  }

  lazy val env = MaidenConfigFactory.env
...
trait JsonAction extends BaseAction with SkipCsrfCheck {
  ...
  private[this] def respondException(e: Throwable) = {
    ...
    val message = underlyingException match {
      case Some(exc) => if (MaidenConfigFactory.env == "dev-local") {
        Map(
    ...
    if (MaidenConfigFactory.env != "dev-local") {
      Future {
        val identity = user match {
          ...
          case _ => {
            val i = new RaygunIdentifier("anonymous@maiden.com")
        ...
        tags.add(MaidenConfigFactory.env)
        tags.add("scala")
...
trait BasicAuthAction extends BaseAction {
  beforeFilter {
    basicAuth("Maiden Restricted") { (username, password) =>
